<template>
    <el-dialog title="修改搜索站点" :visible.sync="showUpdateSearchSite" :close-on-press-escape="false"
...
<script>
    ...
    export default {
        name: "UpdateSearchSiteDialog",
        props: {
            showUpdateSearchSite: {
                type: Boolean,
                default: false
            }
        },
        ...
        methods: {
            onSubmit() {
                this.$refs['form'].validate((valid) => {
                    ...
                    this.$http.put('/user/searchSite', this.form).then(res => {
                        let data = res.data;
                        let code = data.code;
                        if (code !== 30006) {
                            ...
                            TransformData.$emit('cancelUpdateSearchSiteDialog');
        ...
        created() {
            TransformData.$on('updateSearchSite', (item) => {
                this.form = {...item};
            })
        }
    }
